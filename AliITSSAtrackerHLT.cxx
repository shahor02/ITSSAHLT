 ...
 **************************************************************************/
/* $Id: AliITSSATrackerHLT.cxx 32466 2009-05-20 07:51:56Z hristov $ */
...
using std::vector;
ClassImp(AliITSSATrackerHLT)

Bool_t AliITSSATrackerHLT::CheckTrack( const AliExternalTrackParam *t )
{
...
Bool_t AliITSSATrackerHLT::TransportToX( AliExternalTrackParam *t, double x ) const
{
  return CheckTrack(t) && t->PropagateTo( x, t->GetBz() );
}

Bool_t AliITSSATrackerHLT::TransportToPhiX( AliExternalTrackParam *t, double phi, double x ) const
{
...
AliITSSATrackerHLT::AliITSSATrackerHLT()
  :AliTracker(),
   fRecoParam(0),
...
//------------------------------------------------------------------------
AliITSSATrackerHLT::AliITSSATrackerHLT(const Char_t *geom) 
: AliTracker(),
  fRecoParam(0),
  fLayers(new AliHLTITSLayer[AliITSgeomTGeo::kNLayers]),  
  fUseTGeo(2),
...
{
  //--------------------------------------------------------------------
  //This is the AliITSSATrackerHLT constructor
  //--------------------------------------------------------------------
  if (geom) {
...
//------------------------------------------------------------------------
AliITSSATrackerHLT::AliITSSATrackerHLT(const AliITSSATrackerHLT &tracker)
:AliTracker(tracker),
...
//------------------------------------------------------------------------
AliITSSATrackerHLT & AliITSSATrackerHLT::operator=(const AliITSSATrackerHLT &tracker){
  //Assignment operator
  this->~AliITSSATrackerHLT();
  new(this) AliITSSATrackerHLT(tracker);
  return *this;
}
//------------------------------------------------------------------------
AliITSSATrackerHLT::~AliITSSATrackerHLT()
...
void AliITSSATrackerHLT::StartLoadClusters( Int_t NOfClusters )
{
  // !
  delete[] fClusters;
...
void AliITSSATrackerHLT::LoadCluster( const AliITSRecPoint &cluster) 
{
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::LoadClusters(TTree *cTree) {
  //--------------------------------------------------------------------
...
//------------------------------------------------------------------------
void AliITSSATrackerHLT::UnloadClusters() {
  //--------------------------------------------------------------------
...
void AliITSSATrackerHLT::Reconstruct( AliExternalTrackParam *tracksTPC, int *tracksTPCLab, int nTPCTracks )
{
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::Clusters2Tracks(AliESDEvent *event) {
  //--------------------------------------------------------------------
  // This functions reconstructs ITS tracks
...
AliCluster *AliITSSATrackerHLT::GetCluster(Int_t index) const 
{
...
//------------------------------------------------------------------------
void AliITSSATrackerHLT::FollowProlongationTree(AliHLTITSTrack * track ) 
{
...
Int_t AliITSSATrackerHLT::FitOutward(AliHLTITSTrack * track ) 
{
  // FitOutward
...
//------------------------------------------------------------------------
AliHLTITSLayer & AliITSSATrackerHLT::GetLayer(Int_t layer) const
{
  //--------------------------------------------------------------------
  //
  //
...
//------------------------------------------------------------------------
void AliITSSATrackerHLT::CookLabel(AliHLTITSTrack *track,Float_t wrong) const 
{
...
//------------------------------------------------------------------------
void AliITSSATrackerHLT::UpdateESDtrack(AliESDtrack *tESD, AliHLTITSTrack* track, ULong_t flags) const
{
  //
  // Update ESD track
  //
...
//------------------------------------------------------------------------
void AliITSSATrackerHLT::BuildMaterialLUT(TString material) {
  //--------------------------------------------------------------------
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::CorrectForTPCtoITSDeadZoneMaterial(AliHLTITSTrack *t) {
  //--------------------------------------------------------------------
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::CorrectForPipeMaterial(AliHLTITSTrack *t,
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::CorrectForShieldMaterial(AliHLTITSTrack *t,
						 Int_t    shieldindex,
						bool InwardDirection) {
  //-------------------------------------------------------------------
...
//------------------------------------------------------------------------
Int_t AliITSSATrackerHLT::CorrectForLayerMaterial(AliHLTITSTrack *t,
					       Int_t layerindex,
					       bool InwardDirection ){
  //-------------------------------------------------------------------
...
//------------------------------------------------------------------------
Bool_t AliITSSATrackerHLT::LocalModuleCoord(Int_t ilayer,Int_t idet,
...
//------------------------------------------------------------------------
Bool_t AliITSSATrackerHLT::ComputeRoad(AliHLTITSTrack* track,Int_t ilayer,Int_t idet,Double_t &zmin,Double_t &zmax,Double_t &ymin,Double_t &ymax) const {
  //--------------------------------------------------------------------
...
Int_t AliITSSATrackerHLT::PropagateBack(AliESDEvent * /*event*/) 
{
  // dummy
  return 0;
}

Int_t AliITSSATrackerHLT::RefitInward(AliESDEvent * /*event*/ ) 
{
  // dummy
  return 0;
}
...
Bool_t AliITSSATrackerHLT::GetTrackPoint(Int_t /*index*/, AliTrackPoint& /*p*/) const 
{
  // dummy
  return 0;
}

Bool_t AliITSSATrackerHLT::GetTrackPointTrackingError(Int_t /*index*/, 
...
Bool_t AliITSSATrackerHLT::FindSPDTracklets()
{
  // find tracklets for predefined vertex
  ClusterPos2Angles()
}
